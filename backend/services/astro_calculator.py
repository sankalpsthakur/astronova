...
class AstroCalculator:
    ...
    @staticmethod
    def calculate_birth_chart(data: BirthData) -> Dict[str, Dict[str, float]]:
        ...
        for planet, lon in positions.items():
            ...
            chart[planet] = {"sign": sign, "degree": degree}
        return chart

    @staticmethod
    def synastry_aspects(chart1: Dict[str, Dict[str, float]], chart2: Dict[str, Dict[str, float]]) -> List[Dict[str, float]]:
        pos1 = {p: c["degree"] + ZODIAC_SIGNS.index(c["sign"]) * 30 for p, c in chart1.items()}
        pos2 = {p: c["degree"] + ZODIAC_SIGNS.index(c["sign"]) * 30 for p, c in chart2.items()}
