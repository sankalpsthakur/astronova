...
def match(data: SimpleMatchRequest):
    ...
    # Safely get user ID, fallback to anonymous if no JWT
    try:
...
@match_bp.route('/compatibility', methods=['POST'])
@validate_request(SimpleMatchRequest)
def match_compatibility(data: SimpleMatchRequest):
    """Compatibility endpoint that iOS app expects - same logic as main match function"""
    person1 = data.person1
    person2 = data.person2
